...
router.route("/getticket").get(function(req, res){   
   ...
   router.route("/Updateticket").post((req,res)=>{   
    Ticket.findByIdAndUpdate(req.body.id, { title:  req.body.title, type: req.body.type, urgence: req.body.urgence,description:req.body.description, etat:req.body.etat, date:req.body.date   },   
   function(err) {  
    if (err) {  
    res.send(err);  
    return;  
